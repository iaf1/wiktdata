...
class WiktionaryParser(object):
    def __init__(self,language='english',url_preffix='en'):
    ...
    def extract_languages(self,lang_tags):
        unwanted_classes = ['tpos']
        enclose_classes = ['gender']
        items_dict = {}
        for tag in lang_tags.find_all(True, {'class': unwanted_classes}):
            tag.extract()
        for tag in lang_tags.find_all(True, {'class': enclose_classes}):
            tag.replace_with('['+tag.text+']')
        text = lang_tags.text
        key, items_text = text.split(': ')
        items_list = items_text.split(', ')
        items_dict[key.lower()] = items_list
        return items_dict
    
    def parse_translations(self, word_contents, return_tag_translations=False):
        ...
        for translations_index, translations_id, _ in translations_id_list:
            ...
            if span_tag == []:
                pass
            next_sibling = span_tag.parent.find_next_sibling()
            translations = next_sibling.find_all('li')
            translations_tag.append(next_sibling)
            for transl in translations:
                if transl.find_all('dd') == []:
                    translations_dict = dict(translations_dict, **self.extract_languages(transl))
                else:
                    lang = transl.text.split(':')[0]
                    descriptions_dict = {}
                    for descr in transl.find_all('dd'):
                        descriptions_dict = dict(descriptions_dict, **self.extract_languages(descr))
                    translations_dict[lang.lower()] = descriptions_dict
            translations_list.append((translations_index, translations_dict))
        
        if return_tag_translations: return translations_tag
